import { ICubeColourNames } from './types/ICubeColourNames';

class CubeRenderer {
  static #svgTemplate = `<svg class="cube" width="425" height="490" viewBox="0 0 425 490" xmlns="http://www.w3.org/2000/svg">
  ...
  static #cubeColors: { [key in ICubeColourNames]: string } = {
    r: '#ff0000',
    g: '#00aa00',
    b: '#0000ff',
    y: '#ffff00',
  ...
  static #get3FaceSVG(cube3face: ICubeColourNames[]): string {
    if (cube3face.length !== 27) {
      throw new Error('ERROR: Invalid cube color representation.');
    }
    let svg = CubeRenderer.#svgTemplate;
    for (let i = 1; i <= 27; i++) {
      const colorId = '{{COLOR_' + (i < 10 ? '0' : '') + i + '}}';
      const color = CubeRenderer.#cubeColors[cube3face[i - 1]];
      svg = svg.replace(colorId, color);
    }
    return svg;
  }

  static getSVG(cube: ICubeColourNames[]): string[] {
    if (cube.length !== 54) {
      throw new Error('ERROR: Invalid cube color representation.');
    }
    return [
      CubeRenderer.#get3FaceSVG(cube.slice(0, 27)),
      CubeRenderer.#get3FaceSVG(cube.slice(27)),
    ];
  }

  static allowedFaceColours(): ICubeColourNames[] {
    return Object.keys(CubeRenderer.#cubeColors) as ICubeColourNames[];
  }
}
